...
export default function EditProductPage() {
  ...
  useEffect(() => {
    fetch(`/api/products/${id}`).then(r => r.json()).then(p => {
      setForm({ ...p, imageUrl: p.image_url || '' });
    });
  }, [id]);
  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex items-center gap-3">
        ...
        <span className="text-sm font-semibold text-gray-900">Edit Product</span>
      </div>
      <h1 className="text-2xl font-black text-gray-900 mb-6">Edit Product</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-200 p-6 max-w-2xl space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Category">
            <input type="text" value={form.category || ''} onChange={e => set('category', e.target.value)} className="field" />
          </Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Stock (-1 = unlimited)">
            <input type="number" value={form.stock ?? -1} onChange={e => set('stock', parseInt(e.target.value))} className="field" />
          </Field>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Link href="/dashboard/products" className="px-5 py-2.5 rounded-xl text-sm font-semibold text-gray-600 hover:bg-gray-100 transition-colors">
            Cancel
          </Link>
        </div>
      </form>
